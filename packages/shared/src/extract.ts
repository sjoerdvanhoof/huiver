...
function resolveHref(baseDir: string, href: string): string {
  ...
  const joined = baseDir ? posix.join(baseDir, clean) : clean;
...
function findCover(env: EpubEnv): ExtractedCover | null {
  ...
  const ext =
    ...
    ?? posix.extname(item.href).replace(".", "").toLowerCase();
 ...
 */
export function extractBookFromBytes(bytes: Uint8Array, originalName: string): ExtractedBook {
  const ext = posix.extname(originalName).toLowerCase();
  // ".epub" is also stripped from names like "book.epub.zip".
  const baseName = posix.basename(originalName, ext).replace(/\.epub$/i, "").replace(/[_-]+/g, " ").trim();
  ...
  // fflate's decoder rather than TextDecoder: the same call already reads every
  // EPUB entry, and it does not assume a global that React Native may not have.
