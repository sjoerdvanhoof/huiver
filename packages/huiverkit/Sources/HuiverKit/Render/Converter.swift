...
public final class Converter {
    public struct Job: Sendable, Identifiable, Equatable {
        ...
        /// The voice chosen when this was queued, so a queue built up over an
        /// evening does not all switch narrator because the setting changed
        /// while it was waiting.
        public let voiceId: String
        public var id: String { chapterId }

        public init(bookId: String, chapterId: String, voiceId: String) {
            self.bookId = bookId
    ...
    /// Every voice the app has, so a job queued in one voice still renders in
    /// that voice after the selection has moved on. Set by the app once the
    /// voices are loaded.
    ...
    public func convert(book: Book, chapter: Chapter, voice: Voice, options: SamplingOptions) {
        ...
        queue.append(Job(bookId: book.id, chapterId: chapter.id, voiceId: voice.id))
        // Queueing a chapter again is how a failed one is retried, so the old
        // complaint goes now rather than lingering next to a running job.
        Task { [library] in
            try? await library.setRenderError(nil, chapterId: chapter.id, bookId: book.id)
        }
        persist()
    ...
    /// The voice a job should be rendered in: the one it was queued with, or
    /// the current selection when that voice has since been deleted.
    private func voice(for job: Job) -> Voice? {
        voices.first { $0.id == job.voiceId } ?? voice
    }
    ...
    private func persist() {
        UserDefaults.standard.set(
            queue.map { [$0.bookId, $0.chapterId, $0.voiceId] }, forKey: Self.queueKey
        )
    }

    /// Turn what was written to `UserDefaults` back into jobs.
    ///
    /// Its own function so the format — including the older two-element shape —
    /// can be tested without an engine to build a converter around.
    static func decodeQueue(_ saved: [[String]], fallbackVoice: String) -> [Job] {
        saved.compactMap { entry in
            switch entry.count {
            case 3: return Job(bookId: entry[0], chapterId: entry[1], voiceId: entry[2])
            // Written before jobs remembered their voice. Whatever is selected
            // now is the best guess, and is what would have happened anyway.
            case 2: return Job(bookId: entry[0], chapterId: entry[1], voiceId: fallbackVoice)
            default: return nil
            }
        }
    }
    ...
    public func restore(voice: Voice, options: SamplingOptions) {
        ...
        let restored = Self.decodeQueue(saved, fallbackVoice: voice.id)
