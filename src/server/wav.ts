 ...
 * left past it. Never grows the file, and is a no-op on a file that already
 * ends exactly there.
 */
export async function truncateWavData(filePath: string, dataBytes: number): Promise<void> {
  ...
  try {
    await handle.truncate(layout.dataOffset + keep);
    await writeSizes(handle, layout.dataOffset, keep);
    await handle.sync();
  } finally {
    await handle.close();
  }
