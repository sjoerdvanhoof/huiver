...
import { extractBook, htmlToText } from "./extract";
...
async function withTempFile<T>(name: string, data: Uint8Array | string, fn: (file: string) => Promise<T>) {
  const dir = await mkdtemp(path.join(tmpdir(), "huiver-test-"));
...
test("extractBook reads EPUB metadata and spine order", async () => {
  const result = await withTempFile("book.epub", buildEpub(), file => extractBook(file, "book.epub"));
...
test("extractBook splits plain text on headings", async () => {
  ...
  const result = await withTempFile("story.txt", text, file => extractBook(file, "my-story.txt"));
...
test("extractBook falls back to a single part when there are no headings", async () => {
  const result = await withTempFile("flat.txt", body("No headings here at all."), file =>
    extractBook(file, "flat.txt"),
  );
...
test("a dragged EPUB folder (arrives as .zip) is still recognised", async () => {
  ...
  const result = await withTempFile("book.zip", buildEpub(), file => extractBook(file, "book.epub.zip"));
...
test("EPUB nested one level deep inside the zip is found", async () => {
  ...
  const result = await withTempFile("nested.zip", zipSync(entries), file => extractBook(file, "nested.zip"));
  expect(result.title).toBe("The Test Book");
  expect(result.chapters.map(c => c.title)).toEqual(["Chapter One", "Chapter Two"]);
});
...
test("NCX chapter titles win over a repeated <title>", async () => {
  ...
  const result = await withTempFile("ncx.epub", epub, file => extractBook(file, "ncx.epub"));
...
test("several chapters inside one spine file are split at their NCX anchors", async () => {
  ...
  const result = await withTempFile("anchors.epub", epub, file => extractBook(file, "anchors.epub"));
...
test("an unmarked table-of-contents page is dropped", async () => {
  ...
  const result = await withTempFile("linky.epub", epub, file => extractBook(file, "linky.epub"));
  expect(result.chapters.map(c => c.title)).toEqual(["Chapter One"]);
});

test("extractBook rejects a non-EPUB zip", async () => {
  const notAnEpub = zipSync({ "hello.txt": strToU8("hi") });
  await expect(
    withTempFile("bad.epub", notAnEpub, file => extractBook(file, "bad.epub")),
