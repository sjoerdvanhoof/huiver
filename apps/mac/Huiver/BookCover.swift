...
///
/// The gradient hash comes from `Cover` in HuiverKit, so a book without
/// artwork is the same colour here as on the phone and in the browser.
/// Proportions are 2:3, the shape of a paperback; a real cover is filled to
...
enum CoverImages {
    ...
    static func cached(_ url: URL) -> Image? {
        cache.object(forKey: url as NSURL).map(Image.init(nsImage:))
    }
