...
/// Read a passage aloud, and have the book read back in your own voice.
///
...
/// measurably makes a worse clone (see `ReferenceClip`). So the sheet asks for
    ...
    private var introduction: some View {
        VStack(alignment: .leading, spacing: Palette.Space.lg) {
            ...
            if recorder.state == .denied {
                Text("Huiver cannot hear the microphone. Turn it on in Settings › Privacy & Security › Microphone.")
                    .font(.huiverCaption)
                    .foregroundStyle(theme.colors.destructive)
            }
    ...
    private var take: some View {
        VStack(alignment: .leading, spacing: Palette.Space.lg) {
            ...
            Button {
                ...
                Text(cloning ? "Creating…" : "Create voice")
                    .font(.huiverHeading)
                    .frame(maxWidth: .infinity)
                    .padding(.vertical, Palette.Space.md)
            }
...
/// Plays the take back, from the samples rather than from a file the sheet
